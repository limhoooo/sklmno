		...
		<div class="text-center">
			<v-dialog v-model.trim="dialog" width="700">
				<v-card class="borderRadiReset">
					...
					>
						<h2 class="mainWhite mainFontS boldWt">보유처 수정</h2>
...
export default {
	...
	computed: {
		...
		dialog: {
			...
			set(newValue) {
				this.$store.state.HoldStoreMgmtModule.infoDialog = newValue;
			},
		},
	},
	methods: {
		async updateStockFnc() {
			if (!nullValidation(this.formData)) {
				return;
			}
			// 그룹 선택안할시 stockType 을 I 로 보냄
			const result = await this.$store.dispatch(
				'HoldStoreMgmtModule/updateStock',
				this.formData,
			);
			if (result) {
				// 리스트 재호출
	...
	created() {
		if (this.activeGroupData) {
			...
			this.formData.stockName = this.activeGroupData.stockName;
			...
			this.formData.stockId = this.activeGroupData.stockId;
		}
	},
};
</script>
