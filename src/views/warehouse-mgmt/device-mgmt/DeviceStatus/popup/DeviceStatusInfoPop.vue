		...
		<div class="text-center">
			<v-dialog v-model.trim="dialog" width="700">
				<v-card class="borderRadiReset">
					...
					>
						<h2
							v-if="infoDialogData.goodsType === 'DVC'"
							class="mainWhite mainFontS boldWt"
						>
							기기이력 상세조회
						</h2>
						<h2 v-else class="mainWhite mainFontS boldWt">유심이력 상세조회</h2>
					...
					<v-card-text class="pad30">
							...
							<table class="w100P">
								<tr>
									...
									<td>
										<input
											type="text"
											:value="infoDialogData.stockTypeMsg"
											class="cursorDefault"
											readonly
										/>
									</td>
								...
								<tr>
									<td>보유처 <b class="orangefont">Before</b></td>
									<td>
										<input
											type="text"
									...
									<td>보유처 <b class="skybluefont">After</b></td>
									<td>
										<input
											type="text"
						...
						<h2
							v-if="infoDialogData.goodsType === 'DVC'"
							class="mainFontS boldWt mainBlack mb10 mt20"
						>
							기기 정보
						</h2>
						<h2 v-else class="mainFontS boldWt mainBlack mb10 mt20">
							유심 정보
						</h2>
						<div class="new_popTable1">
							<table class="w100P">
...
export default {
	...
	computed: {
		deviceHistory() {
			return this.$store.state.DeviceStatusModule.deviceHistory;
		},
		dialog: {
			get() {
			...
			set(newValue) {
				this.$store.state.DeviceStatusModule.infoDialog = newValue;
			},
		},
	},
	methods: {
		async getDeviceHistory() {
			...
			await this.$store.dispatch(
				'DeviceStatusModule/getRetrieveDeviceHistory',
				data,
			);
			//이력 상태 정보 조회
