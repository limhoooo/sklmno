...
import MgmtTargetDetailPop from './popup/MgmtTargetDetailPop';

export default {
	name: 'MediationReleaseList',
	components: { paging, MgmtTargetDetailPop },
	props: {},
	computed: {
		listData() {
			return this.$store.state.MediationReleaseModule.listData;
		},
		detailData: {
			get() {
			...
			set(newValue) {
				this.$store.state.MediationReleaseModule.detailData = newValue;
			},
		},
		mediationFormData: {
			get() {
				return this.$store.state.MediationReleaseModule.mediationFormData;
			},
			set(newValue) {
				this.$store.state.MediationReleaseModule.mediationFormData = newValue;
			},
		},
		totalCnt() {
			return this.$store.state.MediationReleaseModule.totalCnt;
		},
		pagingData() {
			return this.$store.state.MediationReleaseModule.pagingData;
		},
		filterData: {
			get() {
		...
		perPageCnt: {
			get() {
				return this.$store.state.MediationReleaseModule.filterData.perPageCnt;
			},
			set(newValue) {
				this.$store.state.MediationReleaseModule.filterData.perPageCnt = newValue;
			},
		},
		detailDialog: {
			get() {
		...
		mediationDialog: {
			get() {
				return this.$store.state.MediationReleaseModule.mediationDialog;
			},
			set(newValue) {
				this.$store.state.MediationReleaseModule.mediationDialog = newValue;
			},
		},
	},
	methods: {
		...
		getDeviceHistory(data) {
			...
			this.$store.dispatch(
				'MediationReleaseModule/getRetrieveDeviceHistory',
				dvcId,
			);
		},
