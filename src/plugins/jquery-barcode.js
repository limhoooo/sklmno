...
(function ($) {
	...
	function getDigitEAN(code, type) {
		...
		if (type === 'ean8') {
			...
			for (i = 1; i < 7; i++) {
				result +=
					encodingEAN[intval(code.charAt(i))][intval(seq.charAt(i - 1))];
			}
	...
	function computeEAN(code, type) {
		...
		return (
			code + ((10 - (sum % 10)) % 10).toString() + (addon ? ' ' + addon : '')
		);
	}
	...
	var getDigitDatamatrix = (function () {
		var lengthRows = [
			...
			aLogTab = [
				...
				128,
				45,
				90, // Table of aLog for the Galois field
				180,
				69,
				138,
				57,
		...
		function selectIndex(dataCodeWordsCount, rectangular) {
			...
			var n = 0;
			if (
				(dataCodeWordsCount < 1 || dataCodeWordsCount > 1558) &&
				!rectangular
			) {
				return -1;
			}
			...
			for (i = 0; i < totalRowsCW; i++) {
				...
				for (j = 0; j < totalColsCW; j++) {
					if (i % (rowsRegionCW + 2) === 0) {
						...
						datamatrixTemp[i + 1][j + 1] = 0;
						datamatrixTemp[i + 1][j + 1] =
							datamatrix[i - 1 - 2 * parseInt(i / (rowsRegionCW + 2), 10)][
								j - 1 - 2 * parseInt(j / (colsRegionCW + 2), 10)
							];
					}
				}
			}
	...
	function lecCRgb(r, g, b) {
		return (
			String.fromCharCode(b) + String.fromCharCode(g) + String.fromCharCode(r)
		);
	}
	...
	function digitToBmpRenderer($container, settings, digit, hri, mw, mh) {
		var i,
			...
			c0 = isHexColor(settings.bgColor)
				? lecCHexColor(settings.bgColor)
				: lecCRgb(255, 255, 255),
			c1 = isHexColor(settings.color)
	...
	function digitToCanvasRenderer(
		$container,
		settings,
		digit,
		hri,
		...
		if (settings.showHRI) {
			...
			ctx.fillText(
				hri,
				xi + Math.floor((columns * mw - dim.width) / 2),
				yi + lines * mh + settings.fontSize + settings.marginHRI,
			);
		}
	}
	...
	$.fn.barcode = function (data, type, settings) {
		...
		data = $.extend(
			{ crc: true, rect: false },
			typeof data === 'object' ? data : { code: data },
		);
