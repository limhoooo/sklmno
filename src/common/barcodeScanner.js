const VueBarcodeScanner = {
	install(Vue, options) {
		...
		if (options) {
			...
			attributes.setting.soundSrc =
				options.soundSrc || attributes.setting.soundSrc;
			attributes.setting.scannerSensitivity =
				options.sensitivity || attributes.setting.scannerSensitivity;
			attributes.setting.controlSequenceKeys =
				options.controlSequenceKeys || null;
			attributes.setting.callbackAfterTimeout =
				options.callbackAfterTimeout || false;
		}
		...
		Vue.prototype.$barcodeScanner.getBarcodeChkCnt = () => {
			return attributes.barcodeChkCnt;
		};

		Vue.prototype.$barcodeScanner.resetBarcodeChkCnt = () => {
			attributes.barcodeChkCnt = 0;
		};
		...
		function onInputScanned(event) {
			...
			if (checkInputElapsedTime(Date.now())) {
				...
				if (attributes.setting.requiredAttr) {
					barcodeIdentifier = event.target.attributes.getNamedItem(
						'data-barcode',
					);
				} else {
